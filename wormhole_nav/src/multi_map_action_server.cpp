...
void MultiMapActionServer::execute(const std::shared_ptr<GoalHandle> goal_handle){
    ...
    if(current_map.empty())  current_map="room1"; //default starting map
    ...
    goal_handle->publish_feedback(feedback);
    // define map paths in parameters
    std::string map_path = this->declare_parameter<std::string>("maps." + goal->target_map, "");
    if (map_path.empty()) {
        result->success = false;
    ...
    feedback->current_state = "Navigating to final target...";  // Added missing feedback update
    goal_handle->publish_feedback(feedback);
